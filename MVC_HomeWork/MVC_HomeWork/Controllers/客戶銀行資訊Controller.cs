...
namespace MVC_HomeWork.Controllers
{
    public class 客戶銀行資訊Controller : BaseController
    {
        ...
        {
            var data = await repo.GetTaskAll(); // db.客戶聯絡人.Include(客 => 客.客戶資料);
            if (!String.IsNullOrWhiteSpace(KeyWord))
            {
                data = data.Where(p => p.銀行名稱.Contains(KeyWord) || p.帳戶名稱.Contains(KeyWord)).ToList();
            }
            return PartialView("_銀行資訊List", data);
        }
